...
# 두 팔이 같이 접근 가능한 overlap / relay 후보 영역
# 실제 테스트 후 D3~E6 범위는 바꿔도 됨
RELAY_CANDIDATES = [
    "D3", "E3",
    "D4", "E4",
    "D5", "E5",
...
def move_piece(req: MoveRequest):
    ...
    if start not in MOVE_ALLOWED_CELLS:
        raise HTTPException(status_code=400, detail=f"Invalid move start cell for current 3x3 robot script: {start}")

    if end not in MOVE_ALLOWED_CELLS:
        raise HTTPException(status_code=400, detail=f"Invalid move end cell for current 3x3 robot script: {end}")
